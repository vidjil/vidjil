...
def index():
    ...
    ).select(orderby=db.config.classification | db.config.name)
    used_query = db(db.results_file.config_id > 0).select(
        db.results_file.config_id, distinct=True
    )
    used_configs = [row.config_id for row in used_query]
    classification = db((db.classification)).select()

    mes = "Access config list"
    log.info(
...
def add():
    if auth.can_create_config():
        classification = db((db.classification)).select()
        return dict(
            message=T("Add config"), classification=classification, auth=auth, db=db
        )
...
def add_form():
    ...
    if (
        "config_classification" in request.params
        and request.params["config_classification"] != "-1"
    ):
        classification = db(
            db.classification.id == request.params["config_classification"]
        ).count()
        if classification == 0:
    ...
    log.info(
        "Added config",
        extra={"user_id": auth.user_id, "record_id": config_id, "table_name": "config"},
    )
...
def edit():
    if auth.can_modify_config(int(request.query["id"])):
        ...
        return dict(
            message=T("edit config"), classification=classification, auth=auth, db=db
        )
    return error_message(ACCESS_DENIED)
...
def edit_form():
    ...
    db(db.config.id == request.params["id"]).update(
        name=request.params["config_name"],
        info=request.params["config_info"],
        command=request.params["config_command"],
        fuse_command=request.params["config_fuse_command"],
        ...
        for row in query:
            ...
            row.parent_access = ", ".join(
                ...
                for value in auth.get_access_groups(
                    db.config, request.query["id"], group=row.id
                )
            )
...
def change_permission():
    ...
    if auth.get_group_access(
        db.config, int(request.query["config_id"]), int(request.query["group_id"])
    ):
        auth.del_permission(
            request.query["group_id"],
            PermissionEnum.access.value,
            db.config,
            request.query["config_id"],
        )
        res = {
            "message": "c%s: access '%s' deleted to '%s'"
            % (
                request.query["config_id"],
                PermissionEnum.access.value,
        ...
        res = {
            "message": "c%s: access '%s' granted to '%s'"
            % (
                request.query["config_id"],
                PermissionEnum.access.value,
                db.auth_group[request.query["group_id"]].role,
    ...
    log.admin(
        ...
        extra={
            "user_id": auth.user_id,
            "record_id": request.query["config_id"],
            "table_name": "config",
        },
    )
    return json.dumps(res, separators=(",", ":"))
