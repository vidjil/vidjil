...
from pydal.validators import IS_NOT_IN_DB, IS_EMAIL
...
def edit_form():
    ...
    if email != "":
        if current_value["email"] == email:
            ...
            error = f"You cannot change yourself your email adress. Please contact an administrator to do this change."
            ...
            db.auth_user.email.requires = (
                ...
                IS_NOT_IN_DB(
                    db, "auth_user.email",
                    ignore_common_filters=[current_value["email"]], # Ignore current user
                    error_message="Email already exists"
                )
            )
            new_email, error = db.auth_user.email.validate(email) # Don't work anymore as IS_NOT_IN_DB is include in Field declaration for email value
            if error:
                res = {"success": "false", "message": f"new_email: {error}"}
                log.error(res)
                return json.dumps(res, separators=(",", ":"))
            updated_user["email"] = new_email
            log.debug(f"Admin update email address of user {current_value["id"]} from '{current_value["email"]}' to '{email}'")
