...
from ..modules.sequenceFile import (
    get_associated_sample_sets,
    get_sequence_file_sample_sets,
)
...
def next_sample_set():
    ...
    if "next" in request.query:
        try:
            ...
            same_type_with_permissions = (db.sample_set.sample_type == sample_type) & (
                auth.vidjil_accessible_query(PermissionEnum.read.value, db.sample_set)
            )
            if go_next > 0:
                res = db(
                    (db.sample_set.id > sample_set_id) & (same_type_with_permissions)
                ).select(db.sample_set.id, orderby=db.sample_set.id, limitby=(0, 1))
            else:
                res = db(
                    (db.sample_set.id < sample_set_id) & (same_type_with_permissions)
                ).select(db.sample_set.id, orderby=~db.sample_set.id, limitby=(0, 1))
...
def index():
    ...
    if sample_set is None:
        res = {
            "success": "true",
            "message": f"Could not get a sample set with id {int(request.query['id'])}",
        }
        log.error(res)
        return json.dumps(res, separators=(",", ":"))

    owners = db(
        (db.auth_permission.record_id == request.query["id"])
    ...
    (shared_sets, sample_sets_dict) = get_associated_sample_sets(
        all_sequence_files, [sample_set_id]
    )
    ...
    for row in query:
        ...
        if row.sequence_file.id in shared_sets:
            for elt in shared_sets[row.sequence_file.id]:
                values = {
                    "title": sets_names[elt],
                    "sample_type": sample_sets_dict[elt].sample_type,
                    "id": elt,
                }
    ...
    tag_decorator = tag_utils.TagDecorator(settings.TAG_PREFIX)
...
def all():
    ...
    search, tags = tag_utils.parse_search(request.query["filter"])
    ...
    admin_permissions = [
        ...
        for s in db(
            ...
            & (db.sample_set.id.belongs(set_ids))
    ...
    if "sort" in request.query:
        result = sorted(
            result, key=sort_fields[request.query["sort"]]["sort_call"], reverse=reverse
        )
    else:
        request.query["sort"] = ""
        ...
        for p in data[set_type]:
            ...
            if len(errors) == 0:
                ...
                if p["sample_set_id"]:
                    if auth.can_modify_sample_set(int(p["sample_set_id"])):
                        reset = True
                        rows = db(
                            db[set_type].sample_set_id == p["sample_set_id"]
                        ).select()
                        if len(rows) > 0:
                            ...
                            p["error"].append(
                                f"Could not find a {set_type} with sample_set_id {p['sample_set_id']}"
                            )
                            error = True
                    else:
            ...
            if should_register_tags:
                tag_utils.register_tags(
                    db, set_type, p["id"], p["info"], group_id, reset=reset
                )
...
def custom():
    ...
    select = [
        ...
        db.results_file.id,
        db.results_file.config_id,
        db.results_file.hidden,
        db.sequence_file.sampling_date,
        db.sequence_file.pcr,
        db.config.name,
    ...
    if tags is not None and len(tags) > 0:
        ...
        select = select + [db.tag_ref.record_id, db.tag_ref.table_name, count]
    ...
    for visible_row in query:
        ...
        if visible_row.patient.id is not None:
            # TODO use helper.
            visible_row.names = vidjil_utils.display_names(
                visible_row.patient.sample_set_id,
                visible_row.patient.first_name,
                visible_row.patient.last_name,
            )
        ...
        visible_row.string = [
            visible_row.names,
            visible_row.sequence_file.filename,
            str(visible_row.sequence_file.sampling_date),
            str(visible_row.sequence_file.pcr),
            str(visible_row.config.name),
            str(visible_row.results_file.run_date),
            info,
        ]

    query = query.find(
        lambda row: (vidjil_utils.advanced_filter(row.string, search) or row.checked)
    )

    if config:
        query = query.find(
            lambda row: (
                row.results_file.config_id == config_id
                or (str(row.results_file.id) in request.query["custom_list"])
            )
        )
...
def get_configs_by_classification():
    ...
    if auth.can_process_sample_set(int(request.query["id"])):
        for class_elt in db((db.classification)).select(orderby=db.classification.id):
            configs = db(
                (db.config.classification == class_elt.id)
                & (
                    auth.vidjil_accessible_query(PermissionEnum.read.value, db.config)
                    | auth.vidjil_accessible_query(
                        PermissionEnum.admin.value, db.config
                    )
                )
            ).select(orderby=db.config.name)
...
def get_config_samples(query):
    ...
    for row in query:
        ...
        config_samples_with_times[config_name][sequence_file_id].append(
            (run_date, results_file_id)
        )
    ...
    for config_name, sample_sets_results in config_samples_with_times.items():
        ...
        for results_id_with_times in sample_sets_results.values():
            config_samples[config_name].append(
                max(results_id_with_times, key=lambda item: item[0])[1]
            )
...
def confirm():
    if auth.can_modify_sample_set(int(request.query["id"])):
        ...
        return dict(
            message=T("confirm sample_set deletion"),
            data=data,
            helper=helper,
            auth=auth,
            db=db,
...
def delete():
    if auth.can_modify_sample_set(int(request.query["id"])):
        ...
        if sample_set is None:
            res = {
                "message": "An error occurred. This sample_set may have already been deleted"
            }
            log.error(res)
            return json.dumps(res, separators=(",", ":"))
    ...
    if auth.can_modify_sample_set(int(request.query["id"])):
        ...
        for row in query:
            ...
            permissions = db(
                ...
                & (db.auth_permission.table_name == db.sample_set)
            ).select()
            row.perms = ", ".join(map(lambda x: x.name, permissions))

            row.parent_access = ", ".join(
                str(value)
                for value in auth.get_access_groups(
                    db[sample_type], request.query["id"], group=row.id
                )
            )
            row.read = auth.get_group_access("sample_set", request.query["id"], row.id)

        log.info(
            "load permission page for sample_set (%s)" % request.query["id"],
            extra={
                "user_id": auth.user_id,
                "record_id": request.query["id"],
                "table_name": "sample_set",
        ...
        return dict(query=query, helper=helper, data=data, auth=auth, db=db)
    else:
        res = {"message": ACCESS_DENIED}
        log.error(res)
        return json.dumps(res, separators=(",", ":"))
...
def change_permission():
    if auth.can_modify_sample_set(int(request.query["sample_set_id"])):
        ...
        if error == "":
            if auth.get_group_access(
                "sample_set", sample_set_id, int(request.query["group_id"])
            ):
                auth.del_permission(
                ...
                res = {
                    "message": "access '%s' deleted to '%s'"
                    % (
                        PermissionEnum.access.value,
                        db.auth_group[request.query["group_id"]].role,
                    )
                }
            else:
                auth.add_permission(
...
def multi_sample_stats():
    ...
    data["default_hidden_columns"] = [
        index + 1
        for index, header in enumerate(stats_qc_utils.get_stat_headers().values())
        if header.hidden_by_default
    ]
    display_stats_results, json_stats_data = stats_qc_utils.get_stat_data(
        sample_set_id, results_ids
    )
    ...
    permitted_results_ids = [
        permitted_result.results_file.id for permitted_result in permitted_results
    ]
    ...
    log.info(
        f"load multi sample stats ({str(results_ids)})",
        extra={
            "user_id": auth.user_id,
            "record_id": None,
            "table_name": "results_file",
...
def samplesetById():
    ...
    type = (
        request.query["type"]
        if ("type" in request.query.keys())
        else sampleSet.SET_TYPE_GENERIC
    )
...
def stats():
    ...
    if "sort" in request.query:
        result = sorted(
            result, key=lambda row: row[request.query["sort"]], reverse=reverse
        )
    else:
        result = sorted(result, key=lambda row: row.id, reverse=not reverse)
    ...
    log.info(
        "%s stat list %s" % (request.query["type"], search),
        extra={"user_id": auth.user_id, "record_id": None, "table_name": "sample_set"},
    )
...
def result_files():
    ...
    if sum(permissions) < len(permissions):
        ...
        log.error(
            "An error occurred when creating archive of sample_sets %s"
            % str(sample_set_ids)
        )
        return json.dumps(res, separators=(",", ":"))
    ...
    try:
        with ZipFile(full_path_file, "w") as zipfile:
            ...
            for res in results:
                metadata.append(
                    {
                        "id": res.sample_set.id,
                        "name": helpers[res.sample_set.sample_type].get_name(
                            res[res.sample_set.sample_type]
                        ),
        ...
        log.info(
            "extract results files (%s)" % sample_set_ids,
            extra={
                "user_id": auth.user_id,
                "record_id": None,
                "table_name": "sample_set",
        ...
        log.error(
            f"An error occurred when creating archive of sample_sets {sample_set_ids} : {exception}"
        )
        return json.dumps(res, separators=(",", ":"))
    finally:
        try:
