...
def index():
    ...
    for row in query:
        ...
        permissions = auth.get_group_permissions(
            table_name="sample_set", group_id=row.id, myfilter=permissions_list
        )
        row.access = "".join([PermissionLetterMapping[p].value for p in permissions])

    log.info(
        "access group list",
        extra={"user_id": auth.user_id, "record_id": None, "table_name": "group"},
    )
...
def add_form():
    ...
    if error == "":
        id = db.auth_group.insert(
            role=request.params["group_name"], description=request.params["info"]
        )
        ...
        if group_parent is not None and group_parent != "None":
            parent_list = db(db.group_assoc.second_group_id == group_parent).select(
                db.group_assoc.ALL
            )
            parent = None
            if len(parent_list) > 0:
                for parent in parent_list:
                    db.group_assoc.insert(
                        first_group_id=parent.first_group_id, second_group_id=id
                    )
                    auth.add_permission(
                        parent.first_group_id,
                        PermissionEnum.admin_group.value,
                        db.auth_group,
                        id,
                    )
            else:
                ...
                auth.add_permission(
                    group_parent, PermissionEnum.admin_group.value, db.auth_group, id
                )
        else:
        ...
        log.info(
            res, extra={"user_id": auth.user_id, "record_id": id, "table_name": "group"}
        )
        log.admin(res)
        return json.dumps(res, separators=(",", ":"))
...
@vidjil_utils.jsontransformer
def edit():
    if auth.is_admin() or auth.has_permission(
        PermissionEnum.admin.value, db.auth_group, request.query["id"]
    ):
        group = db.auth_group[request.query["id"]]
        log.info(
            "access group edit form",
            extra={
...
def edit_form():
    ...
    if error == "":
        db(db.auth_group.id == request.params["id"]).update(
            role=request.params["group_name"], description=request.params["info"]
        )
...
def info():
    if auth.can_view_group(int(request.query["id"])):
        ...
        parent_group = db(
            db.group_assoc.second_group_id == request.query["id"]
        ).select()
        ...
        for row in sset_count:
            result[row.auth_user.id]["count"] = (
                row.patient_count + row.run_count + row.generic_count
            )
        ...
        return dict(
            message=T("group info"), result=result, group=group, auth=auth, db=db
        )
    return error_message(ACCESS_DENIED)
...
def invite():
    ...
    if auth.can_modify_group(int(request.query["group_id"])):
        ...
        res = {
            ...
            "message": "user '%s' added to group '%s'"
            % (request.query["user_id"], request.query["group_id"]),
        }
        log.admin(res)
        log.info(
            res,
...
def kick():
    ...
    if auth.can_modify_group(int(request.query["group_id"])):
        ...
        res = {
            "redirect": "group/info",
            "args": {"id": request.query["group_id"]},
            "message": "user '%s' removed from group '%s'"
            % (request.query["user_id"], request.query["group_id"]),
        }
        log.admin(res)
        log.info(
...
def rights():
    if auth.is_admin():
        ...
        if request.query["value"] == "true":
            auth.add_permission(
                group_id, request.query["right"], request.query["name"], 0
            )
            msg = (
                "add '"
                + request.query["right"]
        ...
        res = {"redirect": "group/info", "args": {"id": group_id}, "message": msg}
