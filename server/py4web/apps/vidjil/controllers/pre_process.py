...
def index():
    ...
    ).select(orderby=~db.pre_process.name)
    log.info(
...
def add_form():
    ...
    if error == "":
        pre_process_id = db.pre_process.insert(
            name=request.params["pre_process_name"],
...
def edit_form():
    ...
    if error == "":
        db.pre_process[request.params["id"]].update_record(
            name=request.params["pre_process_name"],
            info=request.params["pre_process_info"],
            command=request.params["pre_process_command"],
        )
...
def confirm():
    if auth.can_modify_pre_process(int(request.query["id"])):
        query = db(
            (db.pre_process.id == request.query["id"])
            & (
                auth.vidjil_accessible_query(
                    PermissionEnum.read_pre_process.value, db.pre_process
                )
        ...
        return dict(
            message=T("confirm pre_process deletion"), query=query, auth=auth, db=db
        )
    return error_message(ACCESS_DENIED)
...
def info():
    if auth.can_modify_sample_set(int(request.query["sample_set_id"])):
        ...
        run = (
            db(db.scheduler_task.id == sequence_file.pre_process_scheduler_task_id)
            .select(db.scheduler_task.ALL)
            .first()
        )

        content = None
        out_folder = settings.DIR_PRE_VIDJIL_ID % int(sequence_file_id)
        if not os.path.exists(out_folder):
            log.error(f"Try to open pre-process info, but did not find {out_folder}")
            return error_message(
                "Output folder does not exist, pre-process did not run correctly"
            )
    ...
    for row in query:
        ...
        row.parent_access = ", ".join(
            ...
            for value in auth.get_access_groups(
                db.pre_process, request.query["id"], group=row.id
            )
        )
...
def change_permission():
    ...
    if error == "":
        if auth.get_group_access(
            db.pre_process,
            int(request.query["pre_process_id"]),
