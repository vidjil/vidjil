...
from ..modules.tag_utils import TagDecorator, parse_search
...
def generic_get_group(element_type):
    ...
    group["patient"] = (
        group_concat + separator + "patient.first_name, ' ', patient.last_name, '#')"
    )
...
def get_group_analyses():
    group = {}
    separator = ", ';', "
    fields = ["analysis_file.sample_set_id", "sample_set.sample_type"]
    group_concat = "GROUP_CONCAT(DISTINCT  " + separator.join(fields)
    group["patient"] = (
        group_concat + separator + "patient.first_name, ' ', patient.last_name, '#')"
    )
...
def base_left():
    return [
        db.sample_set.on(db.sample_set.id == db.auth_permission.record_id),
...
def get_permissions(group_list):
    return db(db.auth_group.id.belongs(group_list)).select(
        db.auth_group.role,
        group_permissions(),
        left=(
...
def get_most_used_tags(group_list):
    left = [
        ...
        db.tag.on(db.tag_ref.tag_id == db.tag.id),
    ]
...
def index():
    ...
    if auth.is_admin() and "data" in request.query:
        ...
        group_list = [
            int(g.id) for g in auth.get_user_groups() + auth.get_user_group_parents()
        ]
    ...
    for r in perm_query:
        if r.auth_group.role not in result:
            ...
            for set_type in ["patient", "run", "set"]:
                result[r.auth_group.role][set_type] = {
                    "count": {
                        "num_sets": 0,
                        "num_samples": 0,
                        "sample_type": "generic" if set_type == "set" else set_type,
                    }
                }
        ...
        result[r.auth_group.role]["permissions"] = (
            ""
            if r._extra[group_permissions()] is None
            else r._extra[group_permissions()].replace(",", " ")
        )
    ...
    group_statuses = (
        "GROUP_CONCAT(DISTINCT scheduler_task.id, ';', scheduler_task.status, '#')"
    )
    ...
    left = base_left() + [
        ...
        db.config.on(db.config.id == db.fused_file.config_id),
        db.analysis_file.on(
    ...
    select = [
        ...
        db.sample_set_membership.sequence_file_id.count(distinct=True).with_alias(
            "num_samples"
        ),
        group_statuses,
    ]
    ...
    for set_type in ["patient", "run", "generic"]:
        ...
        set_query = query & (db[set_type].sample_set_id == db.auth_permission.record_id)
    ...
    for key in queries:  # patient, run, set
        ...
        for r in query:  # 1 or 0 rows
            ...
            statuses = (
                ""
                if r._extra[group_statuses] is None
                else "".join(
                    [
                        s.strip("#").split(";")[1][0]
                        for s in r._extra[group_statuses].strip(",").split(",")
                        if s[-1] == "#"
                    ]
                )
            )
            ...
            fuses = (
                []
                if r._extra[group_fuses[key]] is None
                else [
                    fuse.strip("#").split(";")
                    for fuse in r._extra[group_fuses[key]].strip(",").split(",")
                    if fuse[-1] == "#"
                ]
            )
            ...
            analyses = (
                []
                if r._extra[group_analyses[key]] is None
                else [
                    analysis.strip("#").split(";")
                    for analysis in r._extra[group_analyses[key]].strip(",").split(",")
                    if analysis[-1] == "#"
                ]
            )
    ...
    keys = sorted(
        result.keys(),
        key=lambda x: (
            result[x]["patient"]["count"]["num_sets"]
            + result[x]["run"]["count"]["num_sets"]
            + result[x]["set"]["count"]["num_sets"]
        ),
        reverse=True,
    )
...
def jobs():
    ...
    if (
        auth.is_admin()
        and "group_ids" in request.query
        and request.query["group_ids"] is not None
    ):
    ...
    queries = {}
    for set_type in ["patient", "run", "generic"]:
        set_query = query & (db[set_type].sample_set_id == db.sample_set.id)
