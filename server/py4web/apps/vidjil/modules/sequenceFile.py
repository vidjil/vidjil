...
def get_new_uploaded_filename(data_file, new_filename):
    ...
    new_data_file_name = (
        data_file[: name_pos + 1]
        + base64.b16encode(new_filename).lower()
        + new_filename_ext
    )
...
def get_sequence_file_sample_sets(sequence_file_id):
    ...
    """
    query = db((db.sample_set_membership.sequence_file_id == sequence_file_id)).select(
...
def get_associated_sample_sets(sequence_file_ids, forbidden_ids=[]):
    ...
    for shared in shared_sequences:
        shared_sets[shared.sample_set_membership.sequence_file_id].append(
            shared.sample_set.id
        )
...
def get_sequence_file_config_ids(sequence_file_id):
    ...
    """
    query = db((db.results_file.sequence_file_id == sequence_file_id)).select(
...
def check_space(directory, what):
    ...
    if not enough_space:
        ...
        return controller_utils.error_message(
            "{} are temporarily disabled. System admins have been made aware of the situation.".format(
                what
            )
        )
