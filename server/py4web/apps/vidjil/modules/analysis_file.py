...
def get_analysis_data(sample_set_id):
    ...
    if len(analysis_query) > 0:
        ...
        analysis_path = pathlib.Path(settings.DIR_RESULTS, row.analysis_file)
        f = open(analysis_path, "r", encoding="utf-8")
...
def get_clean_analysis(filehandle):
    ...
    analysis = json.loads(filehandle.read())
    result = vidjil_utils.put_properties_in_dict(
        analysis,
        result,
        {
            "cluster": "clusters",
            ...
            "samples": "samples",
            "saved_system_selected_by_config": "saved_system_selected_by_config",
        },
    )
    return result
...
def get_analysis_info(json_paths, sample_set_id):
    ...
    for analysis in analysis_file:
        ...
        results.append(
            vidjil_utils.extract_fields_from_json(json_paths, None, filename)
        )
    return results
