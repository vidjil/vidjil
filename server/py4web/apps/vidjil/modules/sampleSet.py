...
from .tag_utils import TagDecorator
...
class SampleSet(object):
    ...
    def get_display_name(self, data):
        return SPAN(
            ...
            _class="set_token generic_token",
        )
    ...
    def get_tagged_info(self, data):
        text = self.tag_decorator.decorate(
            self.get_info(data), "tag", self.type, self.get_list_path()
        )
    ...
    def get_stats_tagged_info(self, data):
        text = self.tag_decorator.decorate(
            self.get_info(data), "tag", self.type, self.get_stats_path()
        )
        sanitized_text = self.tag_decorator.sanitize(text)
        return SPAN(sanitized_text, _title=self.get_info(data))
    ...
    def get_splitted_configs(self, data):
        ...
        conf_list = (
            [] if data["_extra"][key] is None else data["_extra"][key].split(",")
        )
            ...
            configs.append(
                str(
                    SPAN(
                        A(
                            c_name,
                            SPAN(_class="icon-chart-bar"),
                            _href=f"index.html?sample_set_id={int(data['sample_set_id'])}&config={c_id}",
                            _type="text/html",
                            _id=f"result_sample_set_{int(data['sample_set_id'])}_config_{c_id}",
                            _class="double_button__part1",
                            _title=f"Display results for config {c_name}",
                            _onclick="event.preventDefault();event.stopPropagation();if(event.which == 2) { window.open(this.href); } else { myUrl.loadUrl(db, { 'sample_set_id' : '%d', 'config' :  %s }, '%s' ); }"
                            % (data["sample_set_id"], c_id, filename),
                        ),
                        SPAN(
                            SPAN(_class="icon-table"),
                            _class="double_button__part2",
                            _title=f"Preview / quality control for config {c_name}",
                            _onclick=f"db.call('sample_set/multi_sample_stats', {{'sample_set_id': {data['sample_set_id']}, 'config_id' : {c_id} }})",
                        ),
                        _type="button",
                        _class="double_button",
                    )
                )
            )
    ...
    def get_groups_string(self, data):
        ...
        display_group_list = ", ".join(
            [group for group in group_list if group != "admin"]
        )
        ...
        fields.append(
            {
                "name": "Name",
                "sort": "name",
                "call": self.get_display_name,
                "sort_call": self.get_name,
                "width": "200px",
                "public": True,
            }
        )
        fields.append(
            {
                "name": "Info",
                "sort": "info",
                "call": self.get_tagged_info,
                "sort_call": self.get_info,
                "width": "300px",
                "public": True,
            }
        )
        fields.append(
            {
                "name": "Results",
                "sort": "confs",
                "call": self.get_config_urls,
                "sort_call": self.get_sort_configs,
                "width": None,
                "public": True,
            }
        )
            ...
            fields.append(
                {
                    "name": "Groups",
                    "sort": "groups",
                    "call": self.get_groups_string,
                    "sort_call": self.get_groups_string,
                    "width": "80px",
                    "public": True,
                }
            )
            fields.append(
                {
                    "name": "Creator",
                    "sort": "creator",
                    "call": self.get_creator_string,
                    "sort_call": self.get_creator,
                    "width": "100px",
                    "public": True,
                }
            )
        ...
        fields.append(
            {
                "name": "Info",
                "sort": "info",
                "call": self.get_stats_tagged_info,
                "sort_call": self.get_info,
                "width": None,
                "public": True,
            }
...
def get_sample_name(sample_set_id):
    ...
    if sample is None or (
        sample.sample_type != sampleSet.SET_TYPE_PATIENT
