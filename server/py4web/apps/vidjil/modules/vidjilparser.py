...
class VidjilWriter(object):
    ...
    def _write(self, prefix, event, value, previous):
        ...
        if previous not in [
            ...
            "start_map",
            "start_array",
        ] and event not in ["end_map", "end_array"]:
            mstr = "," + mstr
...
class VidjilParser(object):
    ...
    def isMatching(self, mbuffer, other):
        if other[1] not in MatchingEvent.__members__:
            return False
        return (mbuffer[0] == other[0]) and (
            mbuffer[1] == MatchingEvent[other[1]].value
        )
    ...
    def _extract(self, parser, writer):
        ...
        for prefix, event, value in parser:
            ...
            if any(
                subelem(prefix, item[0])
                or (
                    subelem(item[0], prefix)
                    and (value is None or subelem(item[0], str(value)))
                )
                for item in self.prefixes
            ):
                bufferOn = any(
                    prefix == item[0] for item in self.prefixes
                ) and self.isStartEvent(event)
                if bufferOn:
                    bufferStart = (prefix, event)
                    saved_previous = previous
                    self._writer.startBuffering()

                if not self._writer.conserveBuffer and any(
                    (item[1].compare(prefix, value)) for item in self.prefixes
                ):
                ...
                previous = event
                if self.writer().buffering and (
                    self.isEndEvent(event)
                    and self.isMatching(bufferStart, (prefix, event))
                    or self._writer.conserveBuffer
                ):
