...
from . import sampleSet, vidjil_utils
...
class SampleSets:
    ...
    def __init__(self, ids):
        ...
        self.ids = ids
        self.sample_sets = db(db.sample_set.id.belongs(ids)).select(
            db.sample_set.sample_type, db.sample_set.id
        )
    ...
    def get_names(self):
        ...
        for stype in self.sample_types:
            ...
            if stype == sampleSet.SET_TYPE_PATIENT:
                patient_ids = db(db[stype].sample_set_id.belongs(stype_ids)).select(
                ...
                set_names = db(db[stype].sample_set_id.belongs(stype_ids)).select(
    ...
    def get_tag_names(self):
        ...
        for stype in self.sample_types:
            stype_ids = self.get_sample_type_ids(stype)
            tags = db(
                (db[stype].sample_set_id.belongs(stype_ids))
