...
class TagExtractor(TagManager):
    ...
    def link_to_group(self, tag_id, group_id):
        db = self.db
        assocs = db(
            (db.group_tag.tag_id == tag_id) & (db.group_tag.group_id == group_id)
        ).select()
        if len(assocs) == 0:
    ...
    def link_to_record(self, tag_id, table, record_id):
        db = self.db
        db.tag_ref.insert(tag_id=tag_id, table_name=table, record_id=record_id)
        db.commit()

    def remove_tags(self, table, record_id):
        db = self.db
        db(
            (db.tag_ref.table_name == table) & (db.tag_ref.record_id == record_id)
        ).delete()
        db.commit()
...
class TagDecorator(TagManager):
    ...
    def decoration(self, ltype, stype, target):
        ...
        return (
            r'<a onclick="event.preventDefault();event.stopPropagation();db.callLinkable(%s)" href="%s" class="%s-link" data-sample-type="%s" data-linkable-type="%s" data-linkable-target-param="%s" data-linkable-name="%s\1">%s\1</a>'
            % (this, target, ltype, stype, ltype, "filter", self.prefix, self.prefix)
        )
    ...
    def decorate(self, text, ltype, stype, target):
        ...
        return re.sub(
            self.expression(),
            self.decoration(ltype, stype, target),
            text,
            flags=re.UNICODE,
        )
...
def get_tags(db, group_ids):
    ...
    return db(
        (db.tag.id == db.group_tag.tag_id) & (db.group_tag.group_id.belongs(group_ids))
