...
from .common import db, log, scheduler
...
WINDOWS_REGEX = re.compile(
    r"==> found (\d+) .*-windows in .* segments .* inside (\d+) sequences"
)
...
def schedule_run(id_sequence, id_config, grep_reads=None):
    ts = time.time()
    data_id = db.results_file.insert(
        sequence_file_id=id_sequence,
    ...
    log.debug(
        f"Prepare job for {(task_id, program, args)} in queue {get_celery_queue_to_use(id_sequence)}"
    )
    run_process.apply_async(
        (task_id, program, args), queue=get_celery_queue_to_use(id_sequence)
    )
    ...
    res = {
        ...
        "message": "[%s] c%s: process requested - %s %s"
        % (data_id, id_config, grep_reads, filename),
    }
    log.info(res)
    return res
    ...
    try:
        ...
        if sequence_file.pre_process_file:
            ...
            preprocess_data = json.load(
                open(settings.DIR_RESULTS + "/" + sequence_file.pre_process_file)
            )
            if (
                "pre_process" in preprocess_data
                and "reads" in preprocess_data
                and "total" in preprocess_data["reads"]
                and len(preprocess_data["reads"]["total"])
            ):
                if (
                    preprocess_data["reads"]["total"][0] != 0
                ):  # Some case with first version of new preperocess pipeline
...
def run_igrec(id_file, id_config, id_data, clean_before=False, clean_after=False):
    ...
    try:
        ...
        res = {
            "message": "[%s] c%s: IgReC FAILED - %s" % (id_data, id_config, out_folder)
        }
        log.error(res)
        raise
...
def run_mixcr(id_file, id_config, id_data, clean_before=False, clean_after=False):
    ...
    try:
        ...
        res = {
            "message": "[%s] c%s: MiXCR FAILED - %s" % (id_data, id_config, out_folder)
        }
        log.error(res)
        raise
    ...
    res = {
        "message": "[%s] c%s: MiXCR finished - %s" % (id_data, id_config, out_folder)
    }
    log.info(res)
...
def run_copy(
    ...
    try:
        ...
        res = {
            "message": "[%s] c%s: 'copy' finished - %s" % (id_data, id_config, filename)
        }
        log.info(res)
        ...
        error_message = (
            f"Error in run_copy : {traceback.format_exc()}\n\nSetting status to Failed."
        )
        log.error(error_message)
        update_task(task_id, STATUS_FAILED)
        raise
...
@scheduler.task()
def run_fuse(
    id_file, id_config, id_data, sample_set_id, clean_before=True, clean_after=False
):
    log.debug(
        f"run_fuse starts for {id_file=} {id_config=} {id_data=} {sample_set_id=}"
    )
    db._adapter.reconnect()
    try:
        out_folder = settings.DIR_OUT_VIDJIL_ID % id_data
        output_filename = (
            settings.BASENAME_OUT_VIDJIL_ID % id_data + "-%s" % sample_set_id
        )
        ...
        res = {
            "message": "[%s] c%s: 'fuse' finished - %s"
            % (id_data, id_config, db.fused_file[id_fuse].fused_file)
        }
        log.info(res)
        ...
        db.close()
        log.debug(
            f"run_fuse ends for {id_file=} {id_config=} {id_data=} {sample_set_id=}"
        )
...
def custom_fuse(file_list):
    ...
    for id in file_list:
        ...
        if result_file.data_file is not None:
            ...
            if seq_file.pre_process_file is not None:
                files += ",%s" % os.path.abspath(
                    settings.DIR_RESULTS + seq_file.pre_process_file
                )
            files += " "
    ...
    try:
        cmd = (
            "python "
            + os.path.abspath(os.path.join(settings.DIR_FUSE, "fuse.py"))
            + " -o "
            + output_file
            + " -t 100 "
            + files
        )
        proc_srvr = xmlrpc.client.ServerProxy(
            "http://%s:%d" % (settings.FUSE_SERVER, settings.PORT_FUSE_SERVER)
        )
...
def schedule_pre_process(sequence_file_id, pre_process_config_id):
    ...
    db.sequence_file[sequence_file_id].update_record(
        pre_process_scheduler_task_id=task_id
    )
    db.commit()
    ...
    res = {
        "redirect": "reload",
        "message": "{%s} (%s): process requested"
        % (sequence_file_id, pre_process_config_id),
    }
    log.info(res)
    return res
...
def run_pre_process(
    ...
    """
    log.debug(
        f"run_pre_process starts for {pre_process_config_id=} {task_id=} {sequence_file_id=}"
    )
    db._adapter.reconnect()
    try:
        sequence_file = db.sequence_file[sequence_file_id]
        db.sequence_file[sequence_file_id].update_record(
            pre_process_flag=STATUS_RUNNING
        )
        db.commit()
        ...
        cmd = pre_process.command.replace(
            "&file1&", settings.DIR_SEQUENCES + sequence_file.data_file
        )
        if sequence_file.data_file2:
            cmd = cmd.replace(
                "&file2&", settings.DIR_SEQUENCES + sequence_file.data_file2
            )
        ...
        try:
            ...
            pathlib.Path(settings.DIR_SEQUENCES + sequence_file.data_file).unlink(
                missing_ok=True
            )
            pathlib.Path(settings.DIR_SEQUENCES + sequence_file.data_file2).unlink(
                missing_ok=True
            )
        except Exception as exception:
            log.error(
                f"[pre_process_id={pre_process_config_id}] [sequence_file_id={sequence_file_id}] Removing files at the end of preprocess failed with exception {exception}."
            )
        ...
        res = {
            "message": "{%s} p%s: 'pre_process' finished - %s"
            % (sequence_file_id, pre_process_config_id, output_file)
        }
        log.info(res)
        ...
        log.error(
            f"Error in run_pre_process {pre_process_config_id} for sequence {sequence_file_id}: {exception}"
        )
        ...
        try:
            db.sequence_file[sequence_file_id].update_record(
                pre_process_flag=STATUS_FAILED
            )
            db.commit()
            update_task(task_id, STATUS_FAILED)
        ...
        db.close()
        log.debug(
            f"run_pre_process ends for {pre_process_config_id=} {task_id=} {sequence_file_id=}"
        )
...
def run_fuse_for_sequence_file(
    sequence_file_id: int, config_id: int, data_id: int
) -> None:
    for row in db(
        db.sample_set_membership.sequence_file_id == sequence_file_id
    ).select():
...
def set_tasks_status_for_sequence_file(sequence_file_id: int, status: str):
    waiting_tasks = db(
        ...
        & (db.scheduler_task.status == STATUS_WAITING)
    ).select()
...
def get_preprocessed_filename(filename1, filename2):
    ...
    if not filename2:
        return filename1
    extension = tools_utils.get_common_suffpref(
        [filename1, filename2], min(len(filename1), len(filename2)), -1
    )
