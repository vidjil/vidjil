...
from pydal import Field, validators
...
db.define_table("auth_group", Field("role", "string"), Field("description", "text"))
...
db.define_table(
    ...
    Field(
        ...
        "reference auth_user",
        ondelete="SET NULL",
        requires=validators.IS_NULL_OR(
            validators.IS_IN_DB(db, "auth_user.id", "%(first_name)s", zero="..")
        ),
...
db.define_table(
    ...
    Field(
        "data_file",
        "upload",
        uploadfolder=settings.DIR_SEQUENCES,
        length=LENGTH_UPLOAD,
    ...
    Field(
        "pre_process_file",
        "upload",
        uploadfolder=settings.DIR_RESULTS,
        length=LENGTH_UPLOAD,
        autodelete=AUTODELETE,
    ),
)

db.define_table("classification", Field("name", "string"), Field("info", "text"))
...
db.define_table("tag", Field("name", "string", length=255, unique=True))
