...
from apps.vidjil.modules import sampleSet, vidjil_utils
...
class DBInitialiser(object):
    ...
    def get_set_dict(self, set_type, sample_set_id, i):
        if set_type == sampleSet.SET_TYPE_PATIENT:
            return dict(
                ...
                last_name=i,
                birth="2010-10-10",
                info="test patient %d #test%d" % (i, i),
                sample_set_id=sample_set_id,
                creator=1,
            )
        d = dict(
            name="%s %d" % (set_type, i),
            info="test %s %d #test%d" % (set_type, i, i),
            sample_set_id=sample_set_id,
            creator=1,
        )
    ...
    def _init_pre_processes(self):
        public_group = self.db(self.db.auth_group.role == "public").select().first()
        pid = self.db.pre_process.insert(
            name="public pre-process",
            command="cat &file1& &file2& > &result&",
            info="concatenate two files",
        )
        auth.add_permission(
            public_group.id, PermissionEnum.access.value, self.db.pre_process, pid
        )
        for i in range(3):
            self.db.pre_process.insert(
                name="test pre-process %d" % i,
                command="dummy &file1& &file2& > &result&",
                info="test %d" % i,
            )
        self.db.pre_process.insert(
            name="pre-process perm",
            command="dummy &file1& &file2& > &result&",
            info="dummy pre_process for permissions",
        )
        self.db.commit()
    ...
    def _init_sequence_files(self):
        ...
        for sample_set in sample_sets:
            for i in range(3):
                ...
                self.db.sample_set_membership.insert(
                    sample_set_id=sample_set.id, sequence_file_id=sfid
                )
                self.db.tag_ref.insert(
                    tag_id=tag_id, table_name=self.db.sequence_file, record_id=sfid
                )
        self.db.commit()
    ...
    def _init_results_files(self):
        ...
        for sf in sequence_files:
            membership = (
                self.db(self.db.sample_set_membership.sequence_file_id == sf.id)
                .select(limitby=(0, 1))
                .first()
            )
            scheduler_task_id = self.db.scheduler_task.insert(
                ...
                start_time=datetime.fromtimestamp(timestamp).strftime(
                    "%Y-%m-%d %H:%M:%S"
                ),
            )
    ...
    def _init_set_association_data(self):
        ...
        for i in range(3):
            ...
            self.db.tag_ref.insert(
                tag_id=tag_id,
                table_name=self.db.sequence_file,
                record_id=sequence_file_id,
            )
            ...
            for t in types:
                ...
                self.db.tag_ref.insert(tag_id=tag_id, table_name=t, record_id=sid)
                self.db.sample_set_membership.insert(
                    sample_set_id=ssid, sequence_file_id=sequence_file_id
                )
        self.db.commit()
