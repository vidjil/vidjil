...
class TestConfigController(unittest.TestCase):
    ...
    def test_index_admin(self):
        ...
        assert result["query"][0]["clipboard"] ==  "{'name': 'Clonality', 'program': 'vidjil', 'classification': '1', 'command': '-c clones -z 100 -r 1 -g germline/homo-sapiens.g -e 1 -2 -w 90 -y all --no-airr', 'fuse_command': '-t 100 -d lenSeqAverage --overlaps', 'info': 'incomplete germlines + larger window (90bp), thus 20bp more on each side. This configuration is advised for studies on IGH clonality'}"
    ...
    def test_add_form_admin_incomplete(self):
        ...
        assert (
            result["message"]
            == "config_name needed, config_command needed, config_fuse_command needed, config_program needed"
        )
    ...
    def test_add_form_admin_config_no_classification(self):
        ...
        assert (
            result["message"]
            == f"config '{db_manipulation_utils.TEST_CONFIG_NAME}' added"
        )
    ...
    def test_edit_form_success(self):
        ...
        assert (
            result["message"]
            == f"config '{db_manipulation_utils.TEST_CONFIG_NAME}_mod' updated"
        )
    ...
    def test_delete_success(self):
        ...
        db_manipulation_utils.log_in_as_default_admin(self.session)
        config_id = db_manipulation_utils.add_config()
        assert db.config[config_id] is not None
        ...
        assert db.config[config_id] is None
    ...
    def test_delete_used_config(self):
        ...
        config_id = db_manipulation_utils.add_config()
        assert db.config[config_id] is not None
        results_file_id = db_manipulation_utils.add_results_file(config_id=config_id)
    ...
    def test_change_permission_access_denied(self):
        ...
        with Omboddle(
            ...
            keep_session=True,
            params={"format": "json"},
            query={"config_id": config_id, "group_id": user_group_id},
        ):
            json_result = config_controller.change_permission()
    ...
    def test_change_permission_granted(self):
        ...
        assert (
            result["message"]
            == f"c{config_id}: access '{PermissionEnum.access.value}' granted to '{db.auth_group[user_group_id].role}'"
        )
        assert auth.get_group_access("config", config_id, user_group_id) is True
    ...
    def test_change_permission_deleted(self):
        ...
        assert (
            result["message"]
            == f"c{config_id}: access '{PermissionEnum.access.value}' deleted to '{db.auth_group[user_group_id].role}'"
        )
        assert auth.get_group_access("config", config_id, user_group_id) is False
