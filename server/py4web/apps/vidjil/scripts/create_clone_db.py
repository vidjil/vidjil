...
def get_last_results(sequence_file, config_ids=None):
    ...
    return db(
        (db.results_file.sequence_file_id == sequence_file)
        & (db.results_file.run_date.belongs(select_max_run))
    ).select()
...
def create_clone_db_for_sequences(sequences, output_file):
    ...
    for sequence in sequences:
        sample_sets = db(
            db.sample_set_membership.sequence_file_id == sequence.id
...
if __name__ == "__main__":
    if len(sys.argv) < 3:
        ...
        \t<id>: group id for whom we want to get the sequence files"
            % sys.argv[0]
        )
        exit(1)
    ...
    create_clone_db_for_sequences(
        get_accessible_sequence_files_in_set_type(
            group_ids, sampleSet.SET_TYPE_PATIENT
        ),
        output_file,
    )
