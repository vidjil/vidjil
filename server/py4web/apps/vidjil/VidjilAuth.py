        ...
        if user:
            ...
            if updated_number_wrong_passwords >= settings.MAX_WRONG_PASSWORDS:
                ...
                if self.send_mail:
                    ...
                    self.send_mail(
                        to=email,
                        subject=f"{settings.SMTP_EMAIL_SUBJECT_START} Your account have been locked",
...
* Updating your password after your account is unlocked if you suspect it may have been compromised""")
    ...
    def get_group_names(self):
        ...
        if self.user is not None:
            memberships = self.db(
                self.table_membership().user_id == self.get_user().get("id")
            ).select(self.table_membership().group_id)
            membership_group_ids = [x.group_id for x in memberships]
            groups = self.db(
                self.table_group().id.belongs(membership_group_ids)
            ).select()
            for group in groups:
    ...
    def get_group_access(self, object_of_action, id, group):
        perm = self.has_permission(
            PermissionEnum.access.value, object_of_action, id, group_id=group
        )
        return perm
    ...
    def get_groups_with_permission(
        self, action: str, object_of_action: str, object_id: int = 0, user_id=None
    ):
        """
    ...
    def get_permission(self, action, object_of_action, object_id=0, user=None):
        ...
        to perform the action on the object_of_action.
        """
        groups_with_permission = self.get_groups_with_permission(
            action, object_of_action, object_id, user
        )
    ...
    def load_permissions(self, action, object_of_action):
        ...
        query = db(self.vidjil_accessible_query(action, object_of_action)).select(
            self.db[object_of_action].id
        )
    ...
    def can_create_sample_set(self, user=None):
        ...
        return self.get_permission(
            PermissionEnum.create.value, "sample_set", user=user
    ...
    def can_create_config(self, user=None):
        ...
        """
        return self.get_permission(
            PermissionEnum.create_config.value, "config", user=user
        ) or self.is_admin(user)
    ...
    def can_create_pre_process(self, user=None):
        ...
        """
        return self.get_permission(
            PermissionEnum.create_pre_process.value, "pre_process", user=user
        ) or self.is_admin(user)
    ...
    def can_modify_patient(self, patient_id, user=None):
        ...
        return exists and (
            self.get_permission(
                PermissionEnum.admin.value, "patient", patient_id, user=user
            )
            or self.is_admin(user)
        )
    ...
    def can_modify_run(self, run_id, user=None):
        ...
        return exists and (
            self.get_permission(PermissionEnum.admin.value, "run", run_id, user=user)
            or self.is_admin(user)
        )
    ...
    def can_modify_subset(self, type, subset_id, user=None):
        ...
        return exists and (
            self.get_permission(PermissionEnum.admin.value, type, subset_id, user=user)
            or self.is_admin(user)
        )
    ...
    def can_modify_sample_set(self, sample_set_id, user=None):
        ...
        perm = self.get_permission(
            PermissionEnum.admin.value, "sample_set", sample_set_id, user
        ) or self.is_admin(user)
        if perm:
            return True
    ...
    def can_modify_user(self, id):
        ...
        return self.is_admin() or (
            (self.user_id == id) and (self.user_id not in settings.LIMITED_ACCOUNTS)
        )
    ...
    def can_modify(self, object_of_action, id, user=None):
        ...
        return exists and (
            self.get_permission(PermissionEnum.admin.value, object_of_action, id, user)
            or self.is_admin(user)
        )
    ...
    def can_view(self, object_of_action, id, user=None):
        ...
        exists = self.exists(object_of_action, id)
        return exists and (
            self.get_permission(PermissionEnum.read.value, object_of_action, id, user)
    ...
    def can_modify_config(self, config_id, user=None):
        ...
        return exists and (
            self.get_permission(
                PermissionEnum.admin_config.value, "config", config_id, user
            )
            or self.is_admin(user)
        )
    ...
    def can_modify_group(self, group_id, user=None):
        ...
        return exists and (
            self.get_permission(
                PermissionEnum.admin_group.value, "auth_group", group_id, user=user
            )
            or self.is_admin(user)
        )
    ...
    def can_process_file(self, object_of_action, id=0, user=None):
        ...
        return exists and (
            self.get_permission(
                PermissionEnum.run.value, object_of_action, id, user=user
            )
            or self.is_admin(user)
        )
    ...
    def can_process_sample_set(self, sample_set_id, user=None):
        ...
        if sample_set is None:
            return False
        perm = self.get_permission(
            PermissionEnum.run.value, "sample_set", sample_set_id, user
        ) or self.is_admin(user)
    ...
    def can_upload_file(self, object_of_action, id=0, user=None):
        ...
        return exists and (
            self.get_permission(
                PermissionEnum.upload.value, object_of_action, id, user=user
            )
            or self.is_admin(user)
        )
    ...
    def can_upload_sample_set(self, id, user=None):
        ...
        perm = self.get_permission(
            PermissionEnum.upload.value, "sample_set", id, user
        ) or self.is_admin(user)
    ...
    def can_use_config(self, config_id, user=None):
        ...
        return exists and (
            self.get_permission(
                PermissionEnum.read_config.value, "config", config_id, user
            )
    ...
    def can_view_sample_set(self, sample_set_id, user=None):
        ...
        perm = self.get_permission(
            PermissionEnum.read.value, "sample_set", sample_set_id, user
        ) or self.is_admin(user)
    ...
    def can_save_patient(self, patient_id, user=None):
        ...
        exists = self.exists("patient", patient_id)
        return exists and (
            self.get_permission(PermissionEnum.save.value, "patient", patient_id, user)
            or self.is_admin(user)
        )
    ...
    def can_save_run(self, run_id, user=None):
        ...
        exists = self.exists("run", run_id)
        return exists and (
            self.get_permission(PermissionEnum.save.value, "run", run_id, user)
            or self.is_admin(user)
        )
    ...
    def can_save_sample_set(self, sample_set_id, user=None):
        ...
        perm = self.get_permission(
            PermissionEnum.save.value, "sample_set", sample_set_id, user
        ) or self.is_admin(user)
    ...
    def get_group_parent(self, group_id):
        db = self.db
        parent_group_list = db((db.group_assoc.second_group_id == group_id)).select(
            db.group_assoc.ALL
        )
    ...
    def vidjil_accessible_query(self, name, table, user_id=None):
        ...
        if isinstance(table, str) and table in self.db.tables():
            ...
            for tablename in tablenames:
                cquery &= self.vidjil_accessible_query(name, tablename, user_id=user_id)
            return cquery
        if (
            not isinstance(table, str)
            and self.has_permission(name, table, 0, user_id)
            and self.has_permission(PermissionEnum.access.value, table, 0, user_id)
        ):
            return table.id > 0
        ...
        everybody_results = db(permission.group_id == 3)(permission.name == name)(
            permission.table_name == table
        ).select(permission.record_id)

        accessible_ids = (
            [i.record_id for i in accessible_ids_results]
    ...
    def del_permission(
        ...
        return self.db(permission.group_id == group_id)(permission.name == name)(
            permission.table_name == str(table_name)
        )(permission.record_id == record_id).delete()
    ...
    def has_permission(
        ...
        if not group_id and self.has_permission(
            name, table_name, record_id, user_id=None, group_id=3
        ):
            return True
    ...
    def update_groups(self):
        ...
        memberships = self.db(table_membership.user_id == self.user_id).select()
    ...
    def __str__(self):
        return "AUTH USER --- %04d - [user id %s] [admin %s] [groups %s]" % (
            self.user_id,
