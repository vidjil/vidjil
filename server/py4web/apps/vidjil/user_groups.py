...
def get_default_creation_group(auth):
    ...
    for group in group_list:
        if auth.is_admin() or auth.has_permission(
            PermissionEnum.create.value, "sample_set", 0, group_id=group.id
        ):
            ...
            if len(parents) > 0:
                ...
                tgt_group = group
            group_dict[tgt_group.id] = {
                "id": tgt_group.id,
                "name": "Personal Group"
                if tgt_group.id == auth.user_group()
                else tgt_group.role,
            }
            elements = db(
                (db.auth_permission.group_id == tgt_group.id)
                & (
                    db.auth_permission.table_name.belongs(
                        "patient", "run", "sample_set"
                    )
                )
    ...
    groups = [
        {"id": group_dict[key]["id"], "name": group_dict[key]["name"]}
...
def get_upload_group_ids(auth):
    ...
    for group in group_list:
        if auth.is_admin() or auth.has_permission(
            PermissionEnum.upload.value, "sample_set", 0, group_id=group.id
        ):
            parents = auth.get_group_parent(group.id)
            if len(parents) > 0:
