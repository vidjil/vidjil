...
parser.add_argument(
    "--init",
    help="Set following arguments as default and start cron",
    action="store_true",
)
parser.add_argument(
    "--delete", help="Unset all cron jobs created by this script", action="store_true"
)
parser.add_argument(
    "--set",
    help="Set values in config file without setting a cron",
    action="store_true",
)
parser.add_argument(
    "--reset", help="Reset config file to default values", action="store_true"
)
parser.add_argument(
    "--start", help="Launch Cron with current settings", action="store_true"
)
...
parser.add_argument(
    "-m", "--monitor", help="host of the monitor that will receive the report"
)
parser.add_argument(
    "-s", "--servername", help="ID of the server entry in the monitor database"
)
parser.add_argument("-k", "--key", help="Path to ssh key to use")
parser.add_argument("-l", "--local", help="Execute scripts on local machine")
parser.add_argument(
    "--cron-user", help="user that will be executing the cron task on this machine"
)
...
def init(
    ...
    set_cron(
        params["python"] + " " + CMD + " --config " + config,
        params["cron_user"],
        params["cron"],
    )
...
def load(file):
    ...
    for line in conf.readlines():
        if line[0] != "#" and not line.isspace():
            ...
            params[splitted_line[0]] = " ".join(
                splitted_line[1 : len(splitted_line)]
            ).rstrip()
...
def set_cron(cmd, user, cron_exp):
    ...
    job.setall(
        cron_arr[0], cron_arr[1], cron_arr[2], cron_arr[3], cron_arr[4], cron_arr[5]
    )
    my_cron.write()
