...
class TestClones < BrowserTest
  ...
  def test_11_download_gethtmlinfo
    ...
    assert ($b.a(:id => "download_info_0").present?), ">> Download button is present in getHTMLinfo returned"
    assert ($b.table(:id => "clone_download_info_0").present?), ">> Information table is present in getHTMLinfo returned"
  end
