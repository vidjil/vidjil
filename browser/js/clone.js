...
Clone.prototype = {
	...
	getPosOfthisClone: function (cloneID) {
		var pos = "unknow";
		for (var i=0; i< m.clones.length; i++) {
			if (m.clones[i].id == cloneID) {
				return i	
			}
		}
		return pos
    ...
    createLocusList: function () {
		...
		// TODO ask for text
		var content = "manual chgmt: <form name='germ'><select NAME='LocusForm' id='germSelector', onChange='m.clones["+ this.getPosOfthisClone(this.id) +"].changeLocus(this.form);'>";
	...
	createSegmentList: function (segment, locus) {
		...
		var content = "<form name="+ segment  +"><select NAME="+segment+" onChange='m.clones["+ this.getPosOfthisClone(this.id) +"].changeSegment(this.form, " + segments[segment][0] + ");'>";
	...
	changeSegment: function (form, segment) {
		// TODO add chgmt of germline in data analysis
		var segments = {"5": ["Vsegment", "V", "germlineV"], "4" : ["Dsegment", "D", "germlineD"], "3": ["Jsegment", "J", "germlineJ"]}
		if ( segment == "5") {
			this.seg[segment]         = form["Vsegment"].value
			this.seg[segment+"start"] = 0 //form["Vsegment"].value
			this.seg[segment+"end"]   = 0 //form["Vsegment"].value			
		} else if ( segment == "4") {
			this.seg[segment]         = form["Dsegment"].value
			this.seg[segment+"start"] = 0 //form["Vsegment"].value
			this.seg[segment+"end"]   = 0 //form["Vsegment"].value
		} else if ( segment == "3") {
			this.seg[segment]         = form["Jsegment"].value
			this.seg[segment+"start"] = 0 //form["Vsegment"].value
			this.seg[segment+"end"]   = 0 //form["Vsegment"].value
		}
		// TODO : insert real value for stats (start, end, evalue, ...)
		this.seg["_evalue"]       = 0 //form["Vsegment"].value
    ...
    getHtmlInfo: function () {
        ...
        html += "<tr><td> locus </td><td colspan='" + time_length + "'>" + this.m.systemBox(this.germline).outerHTML + this.germline + "<div id='listLocus' display='inline'>" + this.createLocusList() +"</div></td></tr>"
        html += "<tr><td> V gene (or 5') </td><td colspan='" + time_length + "'>" + this.getGene("5") + "<div id='listVsegment' display='inline'>" + this.createSegmentList("Vsegment") + "</div></td></tr>"
        html += "<tr><td> (D gene) </td><td colspan='" + time_length + "'>" + this.getGene("4") + "<div id='listDsegment' display='inline'>" + this.createSegmentList("Dsegment") + "</div></td></tr>"
        html += "<tr><td> J gene (or 3') </td><td colspan='" + time_length + "'>" + this.getGene("3") + "<div id='listJsegment' display='inline'>" + this.createSegmentList("Jsegment") + "</div></td></tr>"
