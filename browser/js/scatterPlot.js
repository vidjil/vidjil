...
ScatterPlot.prototype = {
  ...
  * */
  graphMotor: function() {
      ...
      .gravity(function(d) {if (self.dbscanActive) return 0; else return -0.001})
      ...
      */
      ...
      .charge(function(d) {if (self.reloadCharge) {return -1;}}) //Allows to act to the DBSCAN visualization -> clusters with length not equals to 1 are rejected -> independant clusters = independant graphs 
  ...
  unsetGraphLinks: function() {
      ...
      this.grpLinks.selectAll("line").remove(); //Useless links are delete
  },
  ...
  setEdgeContainer: function(allEdges) {
      ...
      this.edgeContainer = allEdges.enter().append("svg:line").attr("class", "line_inactive").on("mouseover", function(d) {self.m.focusEdge(d);});
  },
  ...
  * */
  ...
  switchInitMenu: function() {
      var axisX = document.getElementsByClassName("axis_select_x");
      var axisY = document.getElementsByClassName("axis_select_y");
      var graph_selector = document.getElementsByClassName("axis_select_graph");
      for(var i = 0; i < axisX.length; i++) 
        axisX[i].style.display = this.reinit?"none":"block";
      for (var i = 0; i < axisY.length; i++)
        axisY[i].style.display = this.reinit?"none":"block";
  ...
  */
  activeAllLine: function(bool) {
    var self = this;
    this.grpLinks.selectAll("line").attr("class", bool?"line_active":"line_inactive");
  },
  ...
  * */
  initGrid: function () {
      ...
      if (!self.reinit) this.axis_x_update(x_grid);
      if (!self.reinit) this.axis_y_update(y_grid);
  },
