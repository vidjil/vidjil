...
#include "core/multi_germline.hpp"
...
// GIT_VERSION should be defined in "git-version.h", created by "create-git-version-h.sh", to be
// used outside of releases
...
#define DEFAULT_MIN_CLONES_PER_LOCUS 0
...
#define DEFAULT_MIN_COVER_REPRESENTATIVE                                                           \
    3 // At least 3 reads to support a
...
string usage_examples(char *progname) {
    stringstream ss;
    ss << "Examples (see " DOCUMENTATION ")" << endl
       << "  " << progname
       << " -c clones       -g germline/homo-sapiens.g   -2 -r 1  demo/Demo-X5.fa           # "
          "(basic usage, detect the locus for each read,"
       << endl
       << "                                                                                        "
          "       #  cluster reads and report clones starting from the first read (-r 1),"
       << endl
       << "                                                                                        "
          "       #  including unexpected recombinations (-2), designate V(D)J genes and analyze "
          "CDR3s"
       << endl
       << "  " << progname
       << " -c clones       -g germline/homo-sapiens.g:IGH           demo/Stanford_S22.fasta   # "
          "(restrict to complete recombinations on the IGH locus)"
       << endl
       << "  " << progname
       << " -c clones       -g germline/homo-sapiens.g      -2 -z 20 demo/LIL-L4.fastq.gz      # "
          "(basic usage, output detailed V(D)J analysis on the first 20 clones)"
       << endl
       << "  " << progname
       << " --filter-reads  -g germline/homo-sapiens.g               demo/LIL-L4.fastq.gz      # "
          "(pre-filter, extract all reads that may have V(D)J recombinations)"
       << endl
       << "  " << progname
       << " -c windows      -g germline/homo-sapiens.g   -y 0 -uu -U demo/LIL-L4.fastq.gz      # "
          "(splits all the reads into (large) files depending on the detection of V(D)J "
          "recombinations)"
       << endl
       << "  " << progname
       << " -c designations -g germline/homo-sapiens.g      -2 -X 50 demo/Stanford_S22.fasta   # "
          "(full analysis of each read, here on 50 sampled reads)"
       << endl
       << "  " << progname
       << " -c germlines    -g germline/homo-sapiens.g               demo/Stanford_S22.fasta   # "
          "(statistics on the k-mers)"
       << endl;
...
#define COUNT(n)                                                                                   \
    ...
    app.add_option("reads_file", f_reads,
                   R"Z(reads file, in one of the following formats:
                              ...
                              Uncompressed FASTA/FASTQ reads can be given from standard input with ')Z" STDIN_FILENAME
                   R"Z('.
                 )Z")
        ->required()
        ->type_name("");
    ...
    app.add_option("-c", cmd,
                   "command"

                   "\n  \t\t" COMMAND_CLONES
                   "  \t locus/V(D)J detection, window extraction, clone clustering (default "
                   "command, most efficient, all outputs)"
                   "\n  \t\t" COMMAND_WINDOWS "  \t locus/V(D)J detection, window extraction"
                   "\n  \t\t" COMMAND_DETECT "  \t locus/V(D)J detection"
                   "\n  \t\t" COMMAND_SEGMENT
                   "  \t detailed V(D)J designation, without prior clustering (not as efficient)"
    ...
    app.set_config("--config", "", "read a (.json) config.vidjil file with options")
        ->type_name("FILE")
        ->group(group)
        ->level();
    ...
    app.add_option("--header-sep", read_header_separator, "separator for headers in the reads file")
        ->group(group)
        ->level()
        ->type_name("CHAR='" DEFAULT_READ_HEADER_SEPARATOR "'");
    ...
    app.add_option("--first-reads,-x", max_reads_processed,
                   "maximal number of reads to process ('" NO_LIMIT
                   "': no limit, default), only first reads")
        ->group(group)
        ->transform(string_NO_LIMIT);

    app.add_option("--sampled-reads,-X", max_reads_processed_sample,
                   "maximal number of reads to process ('" NO_LIMIT
                   "': no limit, default), sampled reads")
        ->group(group)
        ->transform(string_NO_LIMIT);

    long long force_read_number = NO_LIMIT_VALUE;
    app.add_option("--read-number", force_read_number,
                   "estimate for read number used in e-value computation (default: '" NO_LIMIT
                   "', count all reads, but takes " +
    ...
    // ----------------------------------------------------------------------------------------------------------------------
    group = "Germline/recombination selection (at least one -g, -V/(-D)/-J, or --find option must "
            "be given)";

    vector<string> multi_germlines;
    app.add_option("--germline,-g", multi_germlines,
                   R"Z(
         -g <.g FILE>(:FOCUS) ...
         ...
         -g PATH
                    human germline preset, shortcut for '-g PATH/)Z" DEFAULT_MULTI_GERMLINE_FILE
                   R"Z(',
    ...
    app.add_option("-V", v_reps_V, "custom V germline multi-fasta file(s)")
        ->group(group)
        ->type_name("FILE");

    app.add_option("-D", v_reps_D, "custom D germline multi-fasta file(s) for V(D)J designation")
        ->group(group)
        ->type_name("FILE");

    app.add_option("-J", v_reps_J, "custom V germline multi-fasta file(s)")
        ->group(group)
        ->type_name("FILE");
    ...
    group += "\n    (using -k option is equivalent to set with -s a contiguous seed with only '#' "
             "characters)";
    ...
    app.add_flag_function(
           ...
           [&](int64_t n) {
               COUNT(n);
               indexType = KMER_INDEX;
           },
           "use a plain index (pre-2019 method) instead of the recommended Aho-Corasick-like "
           "automaton")
        ->group(group)
        ->level();
    ...
    app.add_option("--window,-w", wmer_size,
                   "w-mer size used for the length of the extracted window ('" NO_LIMIT
                   "': use all the read, no window clustering)")
        ->group(group)
        ->level()
        ->transform(string_NO_LIMIT);
    ...
    app.add_option("--e-value-kmer", expected_value_kmer,
                   "maximal e-value for the k-mer heuristics ('" NO_LIMIT
                   "': use same value than '-e')")
        ->capture_default_str()
        ->group(group)
        ->level()
        ->transform(string_NO_LIMIT);
    ...
    app.add_option(
           ...
           "seed, possibly spaced, used for the V/J affectation (default: depends on germline), "
           "given either explicitely or by an alias" PAD_HELP +
               string_of_map(seedMap, " "))
        ->group(group)
        ->level()
        ->type_name("SEED=" DEFAULT_SEED);
    ...
    app.add_flag(
           "-4", multi_germline_unexpected_recombinations_1U,
    ...
    app.add_flag(
           "--not-analyzed-as-clones", keep_unsegmented_as_clone,
           "consider not analyzed reads as clones, taking for junction the complete sequence, to "
           "be used on very small datasets (for example --not-analyzed-as-clones --all -X 20)")
        ->group(group)
        ->level();

    // ----------------------------------------------------------------------------------------------------------------------
    group = "Labeled sequences (windows related to these sequences will be kept even if -r/--ratio "
    ...
    app.add_option("--label", windows_labels_explicit, "label the given sequence(s)")
        ->group(group)
        ->level()
        ->type_name("SEQUENCE");
    app.add_option("--label-file", windows_labels_file, "label a set of sequences given in <file>")
        ->group(group)
        ->level()
        ->type_name("FILE");
    app.add_option("--label-json", windows_labels_json,
                   "read a (.json) label.vidjil (experimental)")
        ->type_name("FILE")
        ->group(group)
        ->level();

    bool only_labeled_windows = false;
    app.add_flag("--label-filter", only_labeled_windows,
                 "filter -- keep only the windows related to the labeled sequences")
        ->group(group)
        ->level();
    ...
    app.add_option("--min-reads,-r", min_reads_clone, "minimal number of reads supporting a clone")
        ->capture_default_str()
        ->group(group);
    app.add_option("--min-ratio", ratio_reads_clone,
                   "minimal percentage of reads supporting a clone")
        ->capture_default_str()
        ->group(group);
    app.add_option("--max-clones", max_clones_id,
                   "maximal number of output clones ('" NO_LIMIT "': no maximum, default)")
        ->group(group);
    ...
    app.add_option("--max-consensus,-y", max_representatives,
                   "maximal number of clones computed with a consensus sequence ('" NO_LIMIT
                   "': no limit)")
        ->capture_default_str()
        ->group(group)
        ->transform(string_NO_LIMIT);
    ...
    app.add_option(
           ...
           "maximal number of clones to be analyzed with a full V(D)J designation ('" NO_LIMIT
           "': no limit, do not use)")
        ->group(group)
        ->type_name("INT=" + string_of_int(max_clones));

    app.add_option("--min-clones-per-locus", min_clones_per_locus,
                   "number of guaranteed output clones per locus (" +
                       to_string(DEFAULT_MIN_CLONES_PER_LOCUS) + ", default)")
        ->group(group)
        ->type_name("INT");
    ...
    app.add_flag_function(
           ...
           "reports and analyzes all clones" PAD_HELP
           "(--min-reads 1 --min-ratio 0 --max-clones " NO_LIMIT " --max-consensus " NO_LIMIT
           " --max-designations " NO_LIMIT ")," PAD_HELP
    ...
    app.add_flag_function(
           ...
           "for large clones, use a sample of the longest and highest quality reads to compute the "
    ...
    app.add_option(
           ...
           "use custom Cost for clone analysis: format \"match, subst, indels, del_end, homo\" "
           "(default " +
               string_of_cost(DEFAULT_SEGMENT_COST) + ")")
        ->group(group)
        ->level()
        ->type_name("COST");
    ...
    app.add_option("--analysis-filter", kmer_threshold,
                   "typical number of V genes, filtered by k-mer comparison, to compare to the "
                   "read ('" NO_LIMIT "': all genes)")
        ->capture_default_str()
        ->group(group)
        ->transform(string_NO_LIMIT)
        ->level();
    ...
    app.add_flag("-d,--several-D", several_D, "try to detect several D (experimental)")
        ->group(group);
    ...
    app.add_option("--cluster-epsilon", epsilon,
                   "minimum required neighbors for automatic clustering. No automatic "
                   "clusterisation if =0.")
        ->capture_default_str()
        ->group(group)
        ->level();
    ...
    app.add_flag("--cluster-save-matrix", save_comp,
                 "generate and save comparative matrix for clustering")
        ->group(group)
        ->level();
    app.add_flag("--cluster-load-matrix", load_comp, "load comparative matrix for clustering")
        ->group(group)
        ->level();

    string forced_edges = "";
    app.add_option("--cluster-forced-edges", forced_edges,
                   "manual clustering -- a file used to force some specific edges")
        ->group(group)
        ->level()
        ->type_name("FILE");
    ...
    app.add_option(
           ...
           "use custom Cost for automatic clustering : format \"match, subst, indels, del_end, "
           "homo\" (default " +
               string_of_cost(DEFAULT_CLUSTER_COST) + ")")
        ->group(group)
        ->level()
        ->type_name("COST");

    // ----------------------------------------------------------------------------------------------------------------------
    group = "Detailed output per read (generally not recommended, large files, but may be used for "
            "filtering, as in -uu -X 1000)";
    ...
    app.add_flag("--out-reads", output_sequences_by_cluster,
                 "output all reads by clones (in " CLONE_DIR "/" CLONE_FILENAME
                 "* files), to be used only on small datasets")
        ->group(group);

    bool output_affects = false;
    app.add_flag("--out-affects,-K", output_affects,
                 "output detailed k-mer affectation for each read (in " AFFECTS_FILENAME
                 " file) (use only for debug, for example -KX 100)")
        ->group(group)
        ->level();
    ...
    app.add_option("--dir,-o", out_dir, "output directory")
        ...
        ->group(group)
        ->type_name("PATH");
    app.add_option("--base,-b", f_basename,
                   "output basename (by default basename of the input file)")
        ->group(group)
        ->type_name("STRING");

    bool out_gz = false;
    app.add_flag("--gz", out_gz, "output compressed .tsv.gz, .fa.gz, and .vidjil.gz files")
        ->group(group)
        ->level();

    bool show_alignments = false;
    app.add_flag("--show-junction", show_alignments,
                 "show germline genes around the junction (experimental, not showing the exact "
                 "alignment)")
        ->group(group)
        ->level();
    ...
    app.add_flag("--no-vidjil", no_vidjil, "do not output clones in .vidjil")
        ->group(group)
        ->level();

    bool output_details = false;
    app.add_flag("--out-details", output_details,
                 "output in AIRR .tsv and in .vidjil more details for clones, even beyond "
                 "--max-designations")
        ->group(group)
        ->level();
    ...
    app.add_flag("--clean-memory", __only_on_exit__clean_memory, "clean memory on exit")
        ->group(group)
        ->level();
    ...
    app.add_flag_function(
           ...
           "filter possibly huge datasets, with a permissive threshold, to extract reads that may "
           "have V(D)J recombinations" PAD_HELP "(equivalent to -c " COMMAND_DETECT
           " --out-detected --e-value " STR(EVALUE_FILTER_READS) " -2)")
        ->group(group);

    app.add_option(
           "--grep-reads",
           [&only_labeled_windows, &windows_labels_explicit,
            &output_sequences_by_cluster](CLI::results_t res) {
               only_labeled_windows = true;
               windows_labels_explicit.push_back(res[0].c_str());
               output_sequences_by_cluster = true;
               return true;
           },
           "output, by clone, reads related to the given window sequence, even when they are below "
           "the thresholds" PAD_HELP "(equivalent to --label SEQUENCE -label-filter --out-reads)")
        ->group(group)
        ->level()
        ->type_name("SEQUENCE");
    ...
    // ----------------------------------------------------------------------------------------------------------------------
    group = "Help";
    app.set_help_flag("--help,-h", "help")->group(group);

    app.add_flag_function(
           "--help-advanced,-H",
           [&](int64_t n) {
               COUNT(n);
               throw CLI::CallForAdvancedHelp();
           },
...
#define IGNORED(options, text)                                                                     \
    app.add_flag_function((options),                                                               \
                          [&](int64_t n) {                                                         \
                              COUNT(n);                                                            \
                              cout << endl << "* WARNING: " << text << endl << endl;               \
                          })                                                                       \
        ->level(3);
#define DEPRECATED(options, text)                                                                  \
    app.add_flag_function((options),                                                               \
                          [&](int64_t n) {                                                         \
                              COUNT(n);                                                            \
                              deprecated = true;                                                   \
                              app.exit(CLI::ConstructionError((text), 1));                         \
                          })                                                                       \
        ->level(3);

    IGNORED("-3", "'-3' is deprecated. This option is ignored and has to be removed, CDR3/JUNCTION "
                  "are now always analyzed on clones under the '--max-designations' threshold");
    ...
    if (cmd == COMMAND_CLONES)
        ...
        return app.exit(
            CLI::ConstructionError("'-c segment' is deprecated, please use '-c designations'", 1));
    else {
    ...
    for (string arg : multi_germlines) {
        ...
        if (stat(arg.c_str(), &buffer) == 0) {
            if (buffer.st_mode & S_IFDIR) {
                ...
                multi_germline_paths_and_files.push_back(
                    make_pair(arg, DEFAULT_MULTI_GERMLINE_FILE));
                continue;
            }
        }
        ...
        multi_germline_paths_and_files.push_back(
            make_pair(extract_dirname(arg), extract_basename(arg, false)));
    }
    ...
    size_t min_cover_representative =
        (size_t)min(min_reads_clone, DEFAULT_MIN_COVER_REPRESENTATIVE);
    ...
    if ((wmer_size < 0) && (wmer_size != NO_LIMIT_VALUE)) {
        return app.exit(
            CLI::ConstructionError("Too small -w. The window size should be positive.", 1));
    }
    ...
    if (mkpath(out_cstr, 0755) == -1) {
        cerr << ERROR_STRING << "Directory creation: " << out_dir << endl;
        perror("");
        return 2;
    }
    ...
    if (output_clone_files) {
        ...
        if (mkpath(outseq_cstr, 0755) == -1) {
            cerr << ERROR_STRING << "Directory creation: " << out_seqdir << endl;
            perror("");
            return 2;
        }
    }
    ...
    switch (command) {
    case CMD_DETECT:
        cout << "Detecting V(D)J recombinations" << endl;
        break;
    case CMD_WINDOWS:
        cout << "Detecting V(D)J recombinations and extracting windows" << endl;
        break;
    case CMD_CLONES:
        cout << "Detecting V(D)J recombinations and analyzing clones" << endl;
        break;
    case CMD_SEGMENT:
        cout << "Designating V(D)J recombinations" << endl;
        break;
    case CMD_GERMLINES:
        cout << "Discovering germlines" << endl;
        break;
    }
    ...
    if (max_clones == NO_LIMIT_VALUE || max_clones > WARN_MAX_CLONES) {
        cout << endl
             << "* WARNING: " << PROGNAME
             << " was run with '--all' option or with a large '--max-designations/-z' option"
             << endl;
    }

    if (command == CMD_SEGMENT) {
        cout << endl
             << "* WARNING: " << PROGNAME << " was run with '-c " COMMAND_SEGMENT "' option"
             << endl;
    }
    ...
    if (max_clones == NO_LIMIT_VALUE || max_clones > WARN_MAX_CLONES || command == CMD_SEGMENT) {
        cout << "* " << PROGNAME << " efficiently extracts windows overlapping the CDR3" << endl
             ...
             << "* Computing accurate V(D)J designations for many sequences ('-c " COMMAND_SEGMENT
                "' or large '-z' values)"
             << endl
    ...
    for (int i = 0; i < argc; i++)
        stream_cmdline << argv[i] << " ";

    SampleOutput output({{"vidjil_json_version", VIDJIL_JSON_VERSION},
                         {"samples",
                          {{"number", 1},
    ...
    for (pair<string, string> path_file : multi_germline_paths_and_files) {
        ...
        try {
            multigermline->buildFromJson(
                json_germlines, GERMLINES_ALL, FIRST_IF_UNCHANGED("", seed, seed_changed),
                FIRST_IF_UNCHANGED(0, trim_sequences, trim_sequences_changed), do_filter_automata);
        } catch (std::exception &e) {
            cerr << ERROR_STRING << PROGNAME << " cannot properly read " << path_file.first << "/"
                 << path_file.second << ": " << e.what() << endl;
    ...
    if (!multi_germline) {
        ...
        if (f_reps_V.size()) {
            ...
            json_germlines["systems"]["custom"] = {
                ...
                {"parameters", {{"seed", seed}, {"search_recombinations", {"5", "3"}}}}};
        }
        // Custom --find germline
        if (f_reps_align.size()) {
            json_germlines["systems"]["align"] = {{"shortcut", "Y"},
                                                  {"recombinations", {{{"1", f_reps_align}}}}};
        }
        try {
            multigermline->buildFromJson(
                json_germlines, GERMLINES_REGULAR, FIRST_IF_UNCHANGED("", seed, seed_changed),
                FIRST_IF_UNCHANGED(0, trim_sequences, trim_sequences_changed), do_filter_automata);
        } catch (std::exception &e) {
    ...
    if (multigermline->getGermlines().size() == 0) {
        return app.exit(CLI::ConstructionError(
            "At least one germline must be given with -g or -V/(-D)/-J.", 1));
    }

    // TODO : make it work?
    //   if (multi_germline_unexpected_recombinations_12) {
    //     Germline *pseudo = new Germline(PSEUDO_UNEXPECTED, PSEUDO_UNEXPECTED_CODE, "", "", "",
    //     trim_sequences, (kmer_threshold != NO_LIMIT_VALUE)); pseudo->seg_method =
    //     SEG_METHOD_MAX12 ; pseudo->set_index(multigermline->index);
    //     multigermline->germlines.push_back(pseudo);
    //   }

    //   if (multi_germline_unexpected_recombinations_1U) {
    //     Germline *pseudo_u = new Germline(PSEUDO_UNEXPECTED, PSEUDO_UNEXPECTED_CODE, "", "", "",
    //     trim_sequences, (kmer_threshold != NO_LIMIT_VALUE)); pseudo_u->seg_method =
    //     SEG_METHOD_MAX1U ;
    //     // TODO: there should be more up/downstream regions for the PSEUDO_UNEXPECTED germline.
    //     And/or smaller seeds ? pseudo_u->set_index(multigermline->index);
    ...
    unsigned long long nb_reads_for_evalue = (expected_value == NO_LIMIT_VALUE) ? 1
                                             : (force_read_number > NO_LIMIT_VALUE)
                                                 ? force_read_number
    ...
    if (max_reads_processed_sample != NO_LIMIT_VALUE) {
        ...
        if (only_nth_read > 1)
            cout << "Processing every " << only_nth_read
                 << (only_nth_read == 2 ? "nd" : (only_nth_read == 3 ? "rd" : "th")) << " read"
                 << endl;
    }
    ...
    if (command == CMD_GERMLINES) {
        ...
        index->labels[KmerAffect::getUnknown()].clear();
        ...
        cout << "  <== " << nb_reads << " reads, " << total_length << " kmers" << endl;
        cout << "\t" << " max" << "\t\t" << "        kmers" << "\n";
        ...
        for (auto it : index->labels) {
            ...
            cout << setw(6) << fixed << setprecision(2)
                 << (float)stats_kmer[key] / total_length * 100 << "%";

            cout << "     " << key << " ";
            for (auto g_it : it.second)
                for (auto loc : g_it->getLocus())
                    cout << loc << " ";
            cout << endl;
        }
    ...
    if (command == CMD_CLONES || command == CMD_WINDOWS || command == CMD_DETECT) {
        ...
        if ((float)nb_total_reads / (float)nb_reads_for_evalue > WARN_RATIO_NB_READS) {
            ...
            cout << "  ! The estimated number of reads was far below the actual number of reads"
                 << endl;
            cout << "  ! There may be false positives, you should run with an higher --read-number"
                 << endl;
        }
        ...
        int nb_segmented_including_too_short =
            we.getNbSegmented(TOTAL_SEG_AND_WINDOW) + we.getNbSegmented(UNSEG_TOO_SHORT_FOR_WINDOW);

        stream_segmentation_info << "  ==> junction detected in "
                                 << nb_segmented_including_too_short << " reads"
                                 << " (" << setprecision(3)
                                 << 100 * (float)nb_segmented_including_too_short / nb_total_reads
                                 << "%)" << endl;
        ...
        stream_segmentation_info << nb_segmented << " reads"
                                 << " (" << setprecision(3) << ratio_segmented << "% of "
                                 << nb_total_reads << " reads)" << endl;

        // warn if there are too few segmented sequences
        if (ratio_segmented < WARN_PERCENT_SEGMENTED) {
            output.add_warning(W20_VERY_FEW_RECOMBINATIONS,
                               "Very few V(D)J recombinations found: " +
                                   fixed_string_of_float(ratio_segmented, 2) + "%",
                               LEVEL_WARN);
            stream_segmentation_info
                << "  ! There are not so many CDR3 windows found in this set of reads." << endl;
            stream_segmentation_info
                << "  ! Please check the causes below and refer to " DOCUMENTATION "." << endl;
        }
        ...
        if (command == CMD_CLONES || command == CMD_WINDOWS) {
            ...
            if (!only_labeled_windows)
                cout << "labeled windows"
                     ...
                     << " and with a ratio >= " << ratio_reads_clone << " ("
            ...
            pair<int, size_t> info_remove =
                windowsStorage->keepInterestingWindows((size_t)min_reads_clone_final);

            cout << "  ==> keep " << windowsStorage->size() << " windows in " << info_remove.second
                 << " reads";
            cout << " (" << setprecision(3) << 100 * (float)info_remove.second / nb_total_reads
                 << "%)  " << endl;
            ...
            if (command == CMD_CLONES) {
                ...
                if (sort_clones.size() == 0) {
                    ...
                    cout << "  ! See the 'Limits to report and to analyze clones' options (-r, "
                            "--min-ratio, -z, --all)."
                         << endl;
                } else {
                    ...
                    if (max_clones == 0)
                        ...
                        cout << "Detailed analysis of at most " << max_clones << " clone"
                             << (max_clones > 1 ? "s" : "");
                    else
                    ...
                    if (output_vdjfa) {
                        out_clones = new_ofgzstream(
                            f_clones, out_gz,
                            "   \t(for sequence post-processing with other software)");
                        cout << "!! To get structured data, do not parse the Fasta headers, but "
                                "rather work on the .vidjil file."
                             << endl;
                    }

                    if (output_clone_files) {
                        cout << "  ==> " << out_seqdir + CLONE_FILENAME + "*"
                    ...
                    for (list<pair<junction, size_t>>::const_iterator it = sort_clones.begin();
                        ...
                        if (global_interrupted) {
                            string msg = "Interrupted after analyzing " + string_of_int(num_clone) +
                                         " clones";
                        ...
                        Germline<KmerAffect> *segmented_germline =
                            windowsStorage->getGermline(it->first);
                        ...
                        oss << "clone-" << setfill('0') << setw(WIDTH_NB_CLONES) << num_clone
                            << "--" << segmented_germline->getCode() << "--" << setfill('0')
                            << setw(WIDTH_NB_READS) << clone_nb_reads << "--" << setprecision(3)
                            << 100 * (float)clone_nb_reads / nb_segmented << "%";
                        string clone_id = oss.str();

                        // Clone label -- Human readable information (is it really useful ?)
                        ostringstream oss_human;
                        oss_human << "#### Clone #" << right << setfill('0')
                                  << setw(WIDTH_NB_CLONES) << num_clone << " – " << setfill(' ')
                                  << setw(WIDTH_NB_READS) << clone_nb_reads << " reads"
                                  << " – " << setprecision(3)
                                  << 100 * (float)clone_nb_reads / nb_segmented << "%  ";
                        ...
                        string window_str =
                            ">" + clone_id + "--window" + " " + label + '\n' + it->first + '\n';

                        bool in_top_by_locus =
                            ((int)nb_output_clones_by_locus[segmented_germline->getCode()] <
                                 min_clones_per_locus &&
                             we.getNbReadsGermline(segmented_germline->getCode()) >=
                                 DEFAULT_LIMIT_RATIO_TOP_PER_LOCUS * nb_segmented);
                        ...
                        if (!windowsStorage->isInterestingJunction(it->first)) {
                            ...
                            if ((max_clones_id >= 0) &&
                                (num_clone >= max_clones_id + 1 && !in_top_by_locus)) {
                                cout << "STOP" << endl;
                                continue;
                            }

                            // If max_representatives is reached, we stop here but still outputs the
                            // window
                            if ((max_representatives >= 0) &&
                                (num_clone >= max_representatives + 1 && !in_top_by_locus)) {
                                if (output_vdjfa)
                        ...
                        if (clone_on_stdout) {
                            ...
                            if (!(num_clone % PROGRESS_POINT_CLONES)) {
                                ...
                                if (!(num_clone % (PROGRESS_POINT_CLONES * PROGRESS_LINE)))
                                    cout << right << setw(10) << num_clone / 1000 << "k clones "
                                         << endl;
                                cout.flush();
                            }
                        }
                        ...
                        if (output_clone_files) {
                            string clone_file_name =
                                out_seqdir + CLONE_FILENAME + string_of_int(num_clone);
                        ...
                        if (verbose) {
                            int total_length = 0;
                            list<Sequence> auditioned =
                                windowsStorage->getSample(it->first, max_auditionned);
                            for (list<Sequence>::const_iterator it = auditioned.begin();
                                 it != auditioned.end(); ++it)
                                total_length += (*it).sequence.size();

                            cout << auditioned.size() << " auditioned sequences, avg length "
                                 << total_length / auditioned.size() << endl;
                        }

                        KmerRepresentativeComputer repComp =
                            windowsStorage->getRepresentativeComputer(
                                it->first, "", min_cover_representative, ratio_representative,
                                max_auditionned);
                        ...
                        // Re-launch also a KmerMultiSegmenter, for control purposes (affectations,
                        // evalue)
                        ...
                        clone->set("_average_read_length",
                                   {fixed_string_of_float(
                                       windowsStorage->getAverageLength(it->first), 2)});
                        clone->set("sequence", kseg->getSequence().sequence);
                        clone->set("_coverage", {repComp.getCoverage()});

                        if (repComp.getCoverage() < WARN_COVERAGE)
                            clone->add_warning(W51_LOW_COVERAGE,
                                               "Low coverage: " +
                                                   fixed_string_of_float(repComp.getCoverage(), 3),
                                               LEVEL_WARN, clone_on_stdout);

                        if (label.length())
                            clone->set("label", label);

                        //$$ If max_clones is reached, we will not run a FineSegmenter but we will
                        //still output the representative
                        bool stop_analysis = ((max_clones >= 0) &&
                                              (num_clone >= max_clones + 1 && !in_top_by_locus) &&
                        ...
                        // FineSegmenter
                        size_t nb_fine_segmented =
                            (size_t)max_clones; // When -1, it will become the max value.
                        ...
                        if (seg.isSegmented()) {
                            ...
                            if (!kseg->box_V->affect.isUnknown() &&
                                !kseg->box_J->affect.isUnknown() &&
                                segmented_germline != Germline<KmerAffect>::getUnseg()) {
                                Tshortcut left_shortcut =
                                              segmented_germline->getRepository()->getShortcut(
                                                  kseg->box_V->affect),
                                          right_shortcut =
                                              segmented_germline->getRepository()->getShortcut(
                                                  kseg->box_J->affect);
                            ...
                            if (cc) {
                                clone->add_warning(
                                    W53_SIMILAR_TO_ANOTHER_CLONE,
                                    "Similar to clone #" + string_of_int(cc) + " - " + code,
                                ...
                                out_edges << code << "  ";
                                out_edges << "Clone #" << setfill('0') << setw(WIDTH_NB_CLONES)
                                          << cc << setfill(' ') << "  ";
                                out_edges << "Clone #" << setfill('0') << setw(WIDTH_NB_CLONES)
                                          << num_clone << setfill(' ') << "  ";
                                out_edges << endl;
                            ...
                            if (output_clone_files) {
                                ...
                                if ((segmented_germline->getSegmentationMethod() ==
                                     SEG_METHOD_MAX12))
                                    *out_clone << ">" << seg.box_J->ref_label << endl
                                               << seg.box_J->ref << endl;
                                *out_clone << endl;
                            }
                        } else {
                            // We remember that the KmerSegmenter detected that sequence and raise
                            // W68
                            seg.code = "Possibly " + segmented_germline->getCode();
                            seg.info = seg.code + seg.info;
                            clone->add_warning("W68",
                                               "V(D)J designation failed, possibly complex or not "
                                               "recombined sequence",
                                               LEVEL_WARN, clone_on_stdout);
                        ...
                        if (output_sequences_by_cluster) // -a option, output all sequences
                        {
                            ...
                            for (list<Sequence>::const_iterator itt = sequences.begin();
                                 itt != sequences.end(); ++itt) {
                                *out_clone << *itt;
                            }
                        }
                    ...
                    if (num_clone > last_num_clone_on_stdout) {
                        cout << endl << endl;
                        cout << "#### Clones "
                             << "#" << setfill('0') << setw(WIDTH_NB_CLONES)
                             << last_num_clone_on_stdout + 1 << " to "
                             << "#" << setfill('0') << setw(WIDTH_NB_CLONES) << num_clone << "..."
                             << endl;
                    }
                    ...
                    if (nb_edges) {
                        cout << "Please review the " << nb_edges << " suggested edge(s) in "
                             << out_dir + EDGES_FILENAME << endl;
                    }

                    cout << "Comparing clone consensus sequences 2 by 2" << endl;
                    list<Sequence> first_representatives =
                        keep_n_first<Sequence>(representatives, LIMIT_DISPLAY);
                    SimilarityMatrix matrix =
                        compare_all(first_representatives, representatives_labels);
                    ...
                    if (sort_clones.size() > 0 && max_clones > 0) {
                        ...
                        SimilarityMatrix matrixLevenshtein =
                            compare_windows(*windowsStorage, Levenshtein,
                                            sort_clones.size() > MAX_CLONES_FOR_SIMILARITY
                                                ? MAX_CLONES_FOR_SIMILARITY
                                                : sort_clones.size());
            ...
            output.set("reads", {{"total", {nb_total_reads}},
        ...
        while (reads->hasNext()) {
            ...
            if (s.isSegmented()) {
                ...
                if (!seg->box_V->affect.isUnknown() && !seg->box_J->affect.isUnknown() &&
                    germline != Germline<KmerAffect>::getUnseg()) {
                    Tshortcut left_shortcut =
                                  germline->getRepository()->getShortcut(seg->box_V->affect),
                              right_shortcut =
            ...
            clone->set("germline", code);
            nb_segmented_by_germline[code]++;
    ...
    std::ostream *out_json = new_ofgzstream(
        f_json, out_gz,
        !no_vidjil ? "\t(main output file, may be opened by the Vidjil web application)"
