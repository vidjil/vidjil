...
The sample graph is hidden with there is only one sample. It shows the X most frequent clones of the sample (this number can be alter with the filter menu).

- The current sample is highlighted with a vertical gray bar, you can change that by clicking on another sample or using =←= and =→=.
