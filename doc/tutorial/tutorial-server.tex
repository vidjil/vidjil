...
already been created by searching their name in the search box at the
upper left corner.

\question{
  You are now on the  creation page for patients, runs, and sets.
  You can create several patients, runs and sets at once there.
...
\begin{verbatim}
    ////////////////////////////
    cy.log("// test_004_prepare_new_patients_and_run")
...
\begin{verbatim}
    ...
    cy.fillRun(0, "id_r1", "run_name 1", "2010-01-01", "run info #tagtest")
...
\begin{verbatim}
    ...
    // Done by the next cypress function call
...
\question{Instead we need to modify the last field on each line. Click on
  ...
  Either click on the correct patient or type the first letters of their
...
\begin{verbatim}
    ...
    var info = " from tutorial server tests #functional #tutorial"

    cy.addSample(preprocess, "nfs", filename1, filename2, "2010-01-01", "Solo; Sample 1" + info, "Last_name_1").as("sample_id_1")
    cy.addSample(preprocess, "nfs", filename1, filename2, "2011-01-01", "Solo; Sample 2" + info, "Last_name_2").as("sample_id_2")
    cy.addSample(preprocess, "nfs", filename1, filename2, "2012-01-01", "Solo; Sample 3" + info, "Last_name_3")
...
\question{When the upload is finished, you can launch the analysis by selecting the
...
\begin{verbatim}
    ...
    cy.get("@sample_id_1").then((sample_id_1) => {
      cy.launchProcess("2", sample_id_1)
...
\begin{verbatim}
      ...
      cy.waitAnalysisCompleted("2", sample_id_1)
    })
...
\begin{verbatim}
    ...
    cy.get("@sample_id_2").then((sample_id_2) => {
      cy.get(`#sample_set_open_${sample_id_2}_config_id_-1`)
        .should("not.exist")
    })
