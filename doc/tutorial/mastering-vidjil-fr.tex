...
séléctionnez le patient \textit{Demo LIL-L3 (tutorial)} patient, et cliquez sur le lien en bas à droite
``see results: multi''.
...
\question{Dans le menu \com{setting}, essayez les différentes options de \com{sample key}.
Les cinq échantillons peuvent être annotés par leur nom, leur date d'échantillonnage
ou par le nombre de jours depuis le premier échantillon.
}
...
\question{Quelle est la longueur moyenne des reads sur les IGH~? et sur les
  ...
  \href{http://www.vidjil.org/doc/algo.html\#unsegmentation}{vidjil.org/doc/algo.html\#unsegmentation}
  }
...
Combien de read ce clone représente-t-il ?
(voir la section du bas, à droite)
}

Il y a plusieurs manières d'afficher la désignation V(D)J.

\question{Dans le menu textit{settings}, sélèctionner \com{length}
pour afficher les zones N en fonction de leur taille.
Revenez à l'état par défaut grâce au réglage \com{sequence (when short)} pour afficher les N en entiers sur les séquences courtes.}

\question{Essayez aussi les options \com{alleles in clone names} : 
en sélectionnant \com{always}, le gene V du clone est affiché comme \com{IGHV3-9*01}.
Revenez à l'état par défaut avec \com{when not *01} pour avoir des désignations V(D)J plus condensées.}
...
\question{Cliquez sur l'étoile à côté de ces clones et choisissez des couleurs de tag,
telles que \texttt{clone 1} ou \texttt{clone 2}. Observez comment cette couleur s'applique sur les autres
visualisations.}
...
Vous pouvez utilisez le bouton \com{get support}  situé dans le menu \com {help} 
pour nous contacter. 
...
\question{ Ouvrez le menu \com{import/export} et cliquez sur \com{export svg}. Le fichier ainsi généré décrit tout les clones (segmentation complete et information sur l'abondance à chaque étape) et peut etre ouvert par n'importe quel tableur pour de plus amples analyses. 

\question{ Ouvrez de nouveau le menu \com{import/export} et cliquez sur \com{export svg}. Ceci export la vue du graphe ou de la grille. La feuille ainsi généré  peut etre ouverte et édité par tout software de traitement d'image(inkscape...)   
  
}  

\vfill
\flushright \it Aurélien Béliard, Aurélie Caillault, Florient Thonier, Mathieu Giraud, Tatiana Rocher, Mikaël Salson
