...
Note that the =browser/index.html= file and the =.data= file should be hosted on the same server.
Otherwise, the server hosting the =.data= file must accept cross-domain queries.
