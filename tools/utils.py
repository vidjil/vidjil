...
# Import defs different ways depending on the place this file is imported from
try:
    import apps.vidjil.modules.defs  # noqa: F401
except Exception:
    import defs
...
def concatenate_with_padding(
    d, d1, d1_size, d2, d2_size, ignore_keys=None, recursive=False, none_init=False
):
...
def common_substring(list):
    ...
    duplicates = [i for i, x in enumerate(table) if table.count(x) == len(list)]
    if len(duplicates) > 0:
...
def get_common_suffpref(line, max_length, order):
    """
    ...
    for i in range(max_length):
        ...
        if order == -1:
            index = i + 1
        if all(map(lambda x: x[order * index] == line[0][order * index], line)):
            common_string += 1
        else:
            break
    if order == 1 or common_string == 0:
        return line[0][:common_string]
    else:
        return line[0][-common_string:]


def interesting_substrings(line, target_length=6, substring_replacement="-"):
    """Return a list with interesting substrings.
    ...
    if max(map(len, line)) <= target_length:
        return line

    min_length = min(map(len, line))

    # Remove prefixes

    common_prefix = get_common_suffpref(line, min_length, 1)

    substrings = [x[len(common_prefix) :] for x in line]
    ...
    common_suffix = get_common_suffpref(line, min_length - len(common_prefix), -1)

    substrings = [x[len(common_prefix) : -len(common_suffix)] for x in line]
    ...
    def check_version(self, filepath):
        ...
        if "vidjil_json_version" in self.d:
            if self.d["vidjil_json_version"] < defs.VIDJIL_JSON_VERSION:
                sys.stderr.write(
                    "! Reading file with old .json version %s\n"
